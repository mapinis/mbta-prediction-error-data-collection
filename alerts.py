...
class AlertEntities(NamedTuple):
    """
...
def add_alert(alert_id: str, severity: int, entities: AlertEntities):
    ...
    with _entity_to_alerts_map_lock:
        ...
        _entity_to_alerts_map.setdefault(entities, set()).add((alert_id, severity))
...
def remove_alert(alert_id: str):
    ...
    with _entity_to_alerts_map_lock:
        if alert_id in _alerts_to_entities_map:
            ...
            _entity_to_alerts_map[entities].remove((alert_id, sev))
            if not _entity_to_alerts_map[entities]:
                _entity_to_alerts_map.pop(entities)


def check_alerts(entities: AlertEntities) -> Set[Tuple[str, int]]:
    """
    ...
    with _entity_to_alerts_map_lock:
        for mask in product([True, False], repeat=4):
            key = AlertEntities(*(v if use else None for use, v in zip(mask, entities)))
